...
In above corrolary, we cannot value the norm of $K$, but in some specitial case, we can do this. Firstly, we need some lemmas.
\begin{lem}
	Let $\A$ be a Banach algebra and $a \in \A$.
	\begin{enumerate}[label=\arabic*)]
		\item If $\alpha \in \rho(a)$, then $\dist{(\alpha,\rho(a))} \geqslant \norm{(a-\alpha)^{-1}}^{-1}$.
...
Combining $1)$ and $2)$ in above lemma, if $N = D + K$ with $\norm{K} < \varepsilon$, then 
\begin{equation*}
