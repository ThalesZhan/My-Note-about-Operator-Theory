...
\end{lem}
\begin{proof}
	Since $\A$ is a $\Cs$, by the continuity of the involution and the multiplication, $a$ is normal. By the Continuous Functional Calculus, we know
	\begin{equation*}
	...
	Let $g_k(z) = z \in C(\sigma(a_k))$ and $g(z) = z \in C(\sigma(a))$, then extending $g_k$ and $g$ on $\cup \sigma(a_n) \cup \sigma(a)$. Therefore, $\norm{g_k} = \norm{a_k}$ and $\norm{g} = \norm{a}$. Take $\alpha \in \sigma(a)$, then since $a_n \sto a$ in norm, for any $0 < \varepsilon < \abs{g(\alpha)}$, there is a $n_0$ s.t. for all $n > n_0$
	\begin{equation*}
