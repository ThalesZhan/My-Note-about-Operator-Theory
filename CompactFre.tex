...
\begin{cor}
	If $N$ is a normal operator on a separable Hilbert space and $\varepsilon > 0$, then there is a diagonalizable normal operator $D$ s.t. $N \cong D \oplus K + K_1 $ and $\sigma(D) = \sigma_e(D) = \sigma_e(N)$, where $K$ and $K_1$ are compact with $\norm{K}, \norm{K_1} < \varepsilon$
\end{cor}
\begin{proof}
	...
	\end{equation*}
	where $D_2$ is a diagonalizable normal operator on a subspace of $\Hs$ with $\sigma(D_2) = \sigma_e(D_2) = \sigma_e(D_1)$. Then for any $\mu_j$, there is a $\lambda_j \in \sigma_e(D_2)$, s.t.
	\begin{equation*}
		\abs{\mu_j - \lambda_j} = \dist{(\mu_j, \sigma_e(D_2))} = \dist{(\mu_j, \sigma_e(N))} < \varepsilon ~(?)
	\end{equation*}
...
\begin{thm}
	If $N$ and $M$ are normal operators on a separable Hilbert space with same essential spectrum, then for any given $\varepsilon > 0$, there is a unitary $U$ s.t. $N-\st{U}MU$ is compact and $\norm{N-\st{U}MU} < \varepsilon$.
\end{thm}
\begin{proof}
	...
	\end{eqution*}
	where $K_1, K$ and $K_1^{'}, K^{'}$ are compact operators with arbitrary small norm. And since $\sigma_e(N) = \sigma_e(M)$, 
	\begin{equation*}
	...
	\end{equation*}
	is compact and its norm can be arbitrary small.
\end{proof}
...
\begin{proof}
	...
	\end{equation*}
	where $\sigma(M_2) = \sigma_e(M_2) = \sigma_e(M) = \sigma_e(N) = \sigma_e(N_2) = \sigma(N_2)$ and the spectrum of $N_1$ and $M_1$ consists of all eigenvalues of $N$ and $M$ with finite multiplicity, and thus $N_1 \cong M_1$. Let $V$ be the unitary s.t. $\st{V}M_1V = N_1$. And by above theorem, there is a sequence of unitaries $\{W_n\}$ s.t.
	\begin{equation*}
