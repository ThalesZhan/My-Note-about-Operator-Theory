...
The compact operators are similar to the finite-dimensional operator. All compact operators can form the unique closed ideal in $\oper$. Moreover, it contains some important classes of operators, like the trace class, which provides an extra topology on $\oper$. This topology plays an important role in the von Neumann algebra. The topology coincides with the $WOT$ in some cases, but it can give a new method to obtain some properties general von Neumann algebras have. For non-compact operators, there is a "weak version" of the compact operator, the Fredholm operator. By researching the Fredholm operators, it provides us an approach to find some "invariant variable" under the compact perturbations. Finally, we have known that finite-dimensional normal operator can always be diagonalizable. But for the infinite-dimensional case, the above statement is not true. By taking some small compact perturbation, the normal operator can be diagonalizable, and this result has some applications of representation theory.
...
\begin{proof}
	...
	\begin{equation*}
		h_0 = \frac{1}{\lambda}Th_0, \text{ i.e. } Th_0 = \lambda h_0 \qedhere
	\end{equation*}
\end{proof}
...
\begin{lem}
	If $T \in \coper$ and a nonzero $\lambda \in \sigma(T)$, then $\ker{(T-\lambda)}$ is finite dimensional.
\end{lem}
\begin{proof}
...
\begin{thm}
	...
	\begin{equation*}
		\Hs \cong \Hs_0 \oplus \bigoplus_{d \in D} \Hs_d^{(k_d)}
	\end{equation*}
	and
	\begin{equation*}
		\A \cong \{~0 \oplus \bigoplus_{d \in D} K_d^{(k_d)} \colon K_d \in \fml{B}_0(\Hs_d)~\}
	\end{equation*}
\end{thm}
\begin{proof}
	Firstly, by above mention, if $\A$ is non-degenerate, there are $\{\Hs_i\}_{i \in I}$ s.t. 
	\begin{equation*}
		\Hs = \bigoplus_{i \in I} \Hs_i~,~ \A|_{\Hs_i} = \fml{B}_0(\Hs_i)
	\end{equation*}
	...
	\begin{equation*}
		\Hs \cong \bigoplus_{d \in D} \Hs_d^{(k_d)}, \A \cong \{~\oplus_{d \in D} K_d^{(k_d)} \colon K_d \in \fml{B}_0(\Hs_d)~\}
	\end{equation*}
...
\begin{thm}
	...
	\begin{equation*}
		\rho \cong \bigoplus \{~\rho_{\zeta}^{(m_{\rho}(\zeta))} \colon \zeta \in \hat{\A}~\}
	\end{equation*}
...
\begin{proof}
	...
	\end{equation*}
	By the Polar Decomposition, $A \in \toper$ is equivalent the $\abs{A} \in \toper$. Thus, $\{\alpha_n\} \in l^{1}(\C)$ is equivalent to $A \in \toper$, and $\norm{A}_1 = \sum \alpha_n$.
...
\begin{equation*}
	\oper \cong \st{(\toper, \norm{\cdot}_1)}
\end{equation*}
Thus, $\oper$ can be equipped with the $weak^{*}$ topology with respect to the $\st{(\toper, \norm{\cdot}_1)}$. The subbasis of a point $A \in \oper$ is like,
\begin{equation*}
...
\begin{proof}
	...
	\begin{eqnarray*}
		...
		&\leqslant& C\norm{h}
	\end{eqnarray*}
	Thus $Kh = \sum \langle h,e_n \rangle Ae_n$ is a bounded operator and clearly it can be approximated by finite rank operators $K_nh=\sum_{k=1}^n \langle h,e_k \rangle Ae_k$, thus $K \in \coper$. But $(A-K)e_n = 0$ for all $n$, that means $\dim{\ker{(A-K)}} = \infty$.
\end{proof}
...
Now, we can apply above theorem to the normal operators. But it can be more general. Since a class of commutative finite rank normal operators can be diagonalizable simultaneously, we want to see the similar result for the infinite dimensional case.
...
\end{lem}
\begin{proof}
	For $1)$, if $A = D + K$, then for any $m \geqslant 1$, $A^m = D^m + K_m$ for some compact operator $K_m$. Therefore, for each polynomial $p$, 
	\begin{equation*}
...
\end{cor}
\begin{proof}
	By above theorem, $N = D_1 + K_1$ for the normal diagonalizable operator $D_1$ and self-adjoint compact operator $K_1$ with $\norm{K_1} < \varepsilon$. Then by the Fredholm Theory, $\sigma_e(D_1) = \sigma_e(N)$. By the \textbf{Theorem} \ref{thm14} in the subsection \textbf{5.4.3} and the separability of the Hilbert space, 
	\begin{equation*}
...
\end{prop}
\begin{proof}
	By above corollary, $N = D_1 + K_1$ for the normal diagonalizable operator $D_1$ and self-adjoint compact operator $K_1$ with $\norm{K_1} < \varepsilon$. And
	\begin{equation*}
		D_1 = D_2 \oplus \bigoplus_{j=1}^{\infty} \mu_j h_j \otimes h_j
	\end{equation*}
	...
	\begin{equation*}
		D_1 = D \oplus \bigoplus_{j=1}^{\infty} (-\lambda_j) g_j \otimes g_j \oplus \bigoplus_{j=1}^{\infty} \mu_j h_j \otimes h_j
	\end{equation*}
	And in fact,
	\begin{equation*}
		D_1 \cong D \oplus \bigoplus_{j=1}^{\infty} (\mu_j-\lambda_j) h_j \otimes h_j
	\end{equation*}
	where $D$ a diagonalizable normal operator on a subspace of $\Hs$ with $\sigma(D) = \sigma_e(D) = \sigma_e(N)$. Put 
	\begin{equation*}
		K = \bigoplus_{j=1}^{\infty} (\mu_j-\lambda_j) h_j \otimes h_j
	\end{equation*}
...
\begin{proof}
	...
	Let $\{\nu_n\}$ and $\{\mu_m\}$ be all eigenvalues of $N$ and $M$ respectively and repeated as often as their multiplicity. Since $\sigma(N) = \sigma_e(N)$, all of them have infinite multiplicity. Moreover, for any $\nu_j$, there is a sequence $\{\mu_{n_j}\}$ converging to it.
