...
Then the structure of Type \RNum{1} algebra has been revealed to some extend. But it can be more explicit. Firstly, for a special case, Type \RNum{1} factor, it has a clear structure.
...
\begin{prop}
	Let $\A$ be a von Neumann algebra and $\{E_i\}$ be a collection of pairwise orthogonal, mutually equivalent projection in $\A$ with $\sum_i E_i = 1$. If $\{E_i\}$ has cardinality $n$ and $\B = \A_{E_1}$, then $\A$ is spacially equivalent to $M_n(\B)$.
\end{prop}
\begin{proof}
...
\begin{proof}
	...
	\end{equation*}
	But since $\A$ is finite and $F_1 \neq 0$, it is  impossible. Therefore, $K$ is infinite. Thus either both $I$ and $K$ are finite or inifite. Moreover, if they are all infinite, because of the separability of $\Hs$, they must have thee same cardinality, $\aleph_0$.\\
...
\subsection{Structure of Type \texorpdfstring{\RNum{1}}{I} Algebras}

Every homogeneous von Numann algebra is Type \RNum{1}. Although the converse is not true, Type \RNum{1} von Neumann algebra can be decomposed into the direct of some homogeneous algebras.

\begin{lem}
	If $\A$ is Type \RNum{1} and $E$ is a nonzero projection in $\A$, then there is an abelian projection $P$ with $P \leqslant E$ and $C_P = C_E$.
\end{lem}
\begin{proof}
	If $F$ is any nonzero projection in $\A$, then $\A_F$ is also Type \RNum{1}. Therefore, if $Z$ is any central projection in $\A$ with $ZF \neq 0$, then there is a nonzero abelian projection is dominated by $ZF$.\\
...
\begin{thm}
	If $\A$ is Type \RNum{1}, then there is a family of pairwise orthogonal central projections $\{Z_i\}$ s.t. $\sum_i Z_i = 1$ and each $\A_{Z_i}$ is homogeneous.
\end{thm}
\begin{proof}
	Claim: $\A$ contains a nonzero central projection $Z$ s.t. $\A_Z$ is homogenrous. \\
	Since $\A$ is Type \RNum{1}, there is a maximal family $\{E_i\}$ of pairwise orthogonal, faithful, abelian projections. And clearly they are mutually equivalent. Put $E = \sum_i E_i$. If $E = 1$, then $\A$ is homogeneous. Assume $E^{\bot} \neq 0$. By preceding lemma, there is a abelian projection $P$ contained in $E^{\bot}$ s.t. $C_{E^{\bot}} = C_P$. By maximality, there is no faithful abelian projection contained in $E^{\bot}$. Thus, $C_P = C_{E^{\bot}} \neq 1$. Put $Z = 1 - C_{E^{\bot}}$, then $ZE^{\bot} = 0$. So $Z = ZE = \sum_i ZE_i$. And also $ZE_i$ is abelian for each $i$. Therefore, $\{ZE_i\}$ is a family of pairwise orthogonal, mutually equivalent abelian projections with $\sum_i ZE_i = Z$, i.e. $\A_Z$ is homogeneous.\\
	By Zorn's Lemma, there is an maximal family of pairwise orthogonal central projections $\{Z_i\}$ s.t. each $\A_{Z_i}$ is homogeneous. Put $Z = \sum_i Z_i$. if $Z^{\bot} \neq 0$. Then $\A_{Z^{\bot}}$ is Type \RNum{1}. By mention above, there is a nonzero central projection $G$ s.t. $(\A_{Z^{\bot}})_G = \A_{{Z^{\bot}}G}$ is homogeneous, which contradicts with the maximality. Therefore, $Z = 1$.
\end{proof}

Then Type \RNum{1} von Neumann algebras can be expressed more explicitly.
\begin{cor}
	If $\A$ is Type \RNum{1}, then there is a family of homogeneous algebras $\{\B_i\}$ and a collection of distinct cardinal numbers $\{n_i\}$ s.t. $\A$ is spatially isomorphic to
	\begin{equation*}
