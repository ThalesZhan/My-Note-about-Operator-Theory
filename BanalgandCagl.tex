...
Let $\fml{H}$ be a Hilbert space. Then $\oper$ is indeed a Banach space. But we have more structure on it. Any two element $S,T \in \oper$ can do multiplication, defined as $ST = S \circ T$, then $ST \in \oper$ by the definition of norm, and moreover $\norm{ST} \leqslant \norm{S}\norm{T}$. Therefore, $\oper$ is an algebra with the extra property of the multiplication, called a Banach algebra.
...
\begin{cor}
	...
	\begin{eqnarray*}
		...
		G &=& G_l \cap G_r = \{~a \in \alg{A} \colon a \text{ is invertible}~\}
	\end{eqnarray*}
...
\begin{defn}
	...
	\end{equation*}	
	And the resolvent of $a$, $\rho(a) = \C \backslash \sigma(a)$.\\
...
\begin{proof}
	...
	\item For any two disjoint open sets $A$ and $B$ in $\C$, $\partial A \cap B = \varnothing$. Thus the boundary of some component of $\tilde{K}$ can not be contained in any component of $\tilde{K}$, i.e. it is contained $K$.
...
By above results, now we can provide the relationships between $\sigma_{\alg{A}}(b)$ and $\sigma_{\alg{B}}(b)$.
...
\begin{thm}
	...
	\begin{enumerate}[label=\arabic*)]
		...
		\item if $G$ is a hole of $\sigma_{\alg{A}}(b)$, then $G \subset \sigma_{\alg{B}}(b)$ or $G \cap \sigma_{\alg{B}}(b) = \varnothing$
	\end{enumerate}
\end{thm}
\begin{proof}
	...
	\item $2)$ holds because of the result of $1)$ and the Maximal Principle.
	\item For $3)$, let $G_1 = G \cap \sigma_{\alg{B}}(b)$ and $G_2 = G \backslash \sigma_{\alg{B}}(b)$. Since $\partial\sigma_{\alg{B}}(b) \subset \sigma_{\alg{A}}(b)$ and $G \cap \sigma_{\alg{A}}(b) = \varnothing$, $G_1 = G \cap \inte{\sigma_{\alg{B}}(b)}$ is open. By the facts that $G_2$ is clearly open and $G = G_1 \bigcup G_2$ and $G_1 \cap G_2 = \varnothing$, either $G_1$ or $G_2$ is empty.
\end{proof}
...
\end{cor}
\begin{proof}
	$1)$ is clearly true since unbounded component does not intersect $\sigma_{\alg{B}}(b)$. $2)$ is because $\C \backslash \sigma_{\alg{A}}(b)$ has no holes. $3)$ is similar as $2)$.
\end{proof}
...
\end{equation*}
Clearly, $p(a)$ is well-defined. But we can do more. If $f$ is an analytic function on $A \subset \C$, then $f$ can be approximated by a sequence of polynomials
\begin{equation*}
...
\end{equation*}
Clearly, by the Cauchy's Integral Formula, this definition is well-defined and is coincided with above definition. But this definition can provide us a convenient method to research the map $f \mapsto f(a)$.
...
\begin{thm}[Riesz Functional Calculus]
	...
	\end{enumerate}
	Moreover, if any map $\tau \colon \hol{(a)} \sto \alg{A}$ satisfies above conditions, then $\tau = \rho$. 
\end{thm}
\begin{proof}
	...
	\begin{equation*}
		\norm{f_n(a)-f(a)} \sto 0
	\end{equation*}
	\item Finally, the uniqueness is because any $f \in \hol{(a)}$ can be approximated uniformly by a sequence of polynomials. Thus, $1)$ and $2)$ means $\tau(p) = \rho(p)$ for any polynomial $p$, and $3)$ provides the fact that $\tau(f) = \rho(f)$ for any $f \in \hol{(a)}$.
\end{proof}
\begin{rem}
	we have mentioned that the integral definition is coincided with the convergent definition. In fact, by $2)$, this statement can be proved rigorously.
\end{rem}
...
\begin{prop} \label{prop10}
	Let $\alg{A}$ be a Banach algebra and $a \in \alg{A}$. $\sigma(a) = F_1 \cup F_2$, where $F_1$ and $F_2$ are disjoint non-empty closed sets. Then there is a non-trial idempotent $e$, i.e. $e^{2}=e$, s.t.
	\begin{enumerate}[label=\arabic*)]
		\item if $ab=ba$, then $eb=be$.
		\item if $a_1=ae$ and $a_2=a(1-e)$, then $a_1a_2=a_2a_1=0$.
		\item $\sigma(a_1)=F_1 \cup \{0\}$ and $\sigma(a_2)=F_2 \cup \{0\}$.
	\end{enumerate}
\end{prop}
\begin{proof}
	...
	\item Let $f_1(z)=zf(z)$ and $f_2(z)=z(1-f(z))$. Then $a_j = f_j(a)$ for $j=1,2$. Then by the Spectral Mapping Theorem $\sigma(a_j) = f_j(\sigma(a_j)) = F_j \cup \{0\}$.
\end{proof}

\begin{cor}
	Let $\Hs$ be a Hilbert space and $T \in \oper$. If $\sigma(T) = F_1 \cup F_2$, where $F_1$ and $F_2$ are disjoint non-empty closed sets, then there are closed subspaces $\Hs_1$ and $\Hs_2$ with $\Hs = \Hs_1 \oplus \Hs_2$, s.t.
	\begin{enumerate}[label=\arabic*)]
...
\end{cor}
\begin{proof}
	From above proposition, $e$ is actually the projection from $\Hs$ to $\Hs_1$. Therefore by the result in above proposition, $1)$ clearly holds.\\
	For $2)$, since $e$ is in fact the identity from $\Hs_1$ to $\Hs_1$, similarly, by the Spectral Mapping Theorem, $\sigma(T_j) = F_j$.
\end{proof}
...
Then we can see the algebraic structure on a \Cs completely determine its norm topology.
...
\begin{prop} \label{prop8}
	Let $\rho \colon \A \sto \B$ be a $*$-homomorphism between two $C^{*}$-algebras.
	\begin{enumerate}[label=\arabic*)]
...
\begin{prop}
	If $h \colon \A \sto \C$ is an algebraic homomorphism on a \Cs $\A$. 
	\begin{enumerate}[label=\arabic*)]
...
\begin{proof}
	...
	\begin{equation*}
		\sigma(a) = \sigma_{\Cg{a}}(a) \subset \R \qedhere
	\end{equation*}
\end{proof}
...
\end{thm}
\begin{proof}
	Firstly, $\Gamma$ is a $*$-homomorphism, since the result in \textbf{Theorem} \ref{thm5} and
	\begin{equation*}
...
\begin{proof}
	...
	\begin{eqnarray*}
		...
		&=& p(z,\clo(z))
	\end{eqnarray*}
	That means that $\rho$ maps the polynomials in $\A$ to polynomials in $\sigma(a)$. Therefore, $\rho$ is unique.
\end{proof}
...
We have known that self-adjoint elements play a important role in a \Cs $\A$. The self-adjoint element $a$ in $\A$ is like the real number in $\C$, and the relationship between them can be revealed by the fact $a \in \Rea{\A}$ if and only if $\sigma(a) \subset \R$. The converse is obtained by the Continuous Functional Calculus. In fact, Continuous functional calculus can provide more relation between the element in $\A$ and the element in $\C$, like positivity.
...
\end{thm}
\begin{proof}
	All of these can be done by the functional calculus. And our goal is to find some functions in $C(\sigma(a))$ to complete these.
...
\end{prop}
\begin{proof}
	For $1)$, let $f(x)= \sqrt[n]{x}$ defined on $\sigma(a) \subset \R$, then $b=f(a)$ satisfying $ a = b^{n}$.
...
\begin{proof}
	...
	\item For $a \in \I$, by the corollary in above subsection, we know there is a $u \in \A$ s.t. $a=u\abs{a}^{\frac{1}{2}}$. By $1)$, $\abs{a}^{\frac{1}{2}} \in \I$. Therefore,
	\begin{equation*}
...
\begin{thm} \label{thm6}
	Let $\A$ be a $\st{C}$-algebra.
	\begin{enumerate}[label=\arabic*)]
		\item If $\I$ is a closed left ideal of $\A$ and $\B = \I \cap \st{\I}$, then $\B$ is a hereditary subalgebra of $\A$.
		\item If $\B$ is a hereditary subalgebra of $\A$ and $\I = \{x\in \A \colon \st{x}x \in \B\}$, then $\I$ is a closed left ideal of $\A$.
		\item If $\I$ is a closed left ideal of $\A$ and $\B = \I \cap \st{\I}$, then $\I = \{x\in \A \colon \st{x}x \in \B\}$.
		\item If $\B$ is a hereditary subalgebra of $\A$ and $\I = \{x\in \A \colon \st{x}x \in \B\}$, then $\B = \I \cap \st{\I}$.
	\end{enumerate}
\end{thm}
\begin{proof}
	...
	\item For $4)$, if $x \in \I_{+}$, then $x^2 \in \B$. So $x = \sqrt{x^2} \in \B_{+}$. Conversely, $x \in \B_{+}$ means $\st{(x^{\frac{1}{2}})}(x^{\frac{1}{2}}) = x \in \B$, $x^{\frac{1}{2}} \in \I_{+}$. Therefore, $\B_{+} = \I_{+}$. Thus, $\B = \I \cap \st{\I}$. 
\end{proof}
...
\begin{proof}
	...
	\item The involution defined on the quotient algebra is well-defined and satisfies the conditions since $\I$ is self-adjoint. We just need to show the induced norm satisfies the $\st{C}$-identity. For $a \in \A$, we have $\norm{\st{a}+\I} = \norm{a+\I}$, and 
	\begin{eqnarray*}
		\norm{a+\I}^2 &=& \norm{(\st{a}+\I)(a+\I)} \\
		&\leqslant& \norm{\st{a}+\I}\norm{a+\I} \\
		&=& \norm{a+\I}^2
...
Then it can provide a \Cs with some similar results as general rings have. These results can also show us how the algebraic structure in a \Cs affect the topological structure. In my opinion, for researching a \Cs, researching the algebraic structure may be more important.
...
\begin{prop}
	...
	\begin{enumerate}[label=\arabic*)]
		...
		\begin{equation*}
			\abs{\phi(\st{y}x)}^2 \leqslant \phi(\st{x}x)\phi(\st{y}y)
		\end{equation*}
...
\end{prop}
\begin{proof}
	For $1)$, it can easily check that $\langle x,y \rangle = \phi(\st{y}x)$ is a semi-inner product, thus by the CBS inequality, above inequality is true.
...
\begin{proof}
	...
	\begin{enumerate}[label=\arabic*)]
		...
		\end{equation*}
		\item Constructing $\Hs_{\phi}$: $\langle \cdot, \cdot \rangle$ is just a semi-inner product on $\A$, thus we need to make it be non-degenerate. The nature method is constructing a equivalent relation or a closed ideal, and then inducing a quotient space. Naturally, we define
		\begin{equation*}
...
\end{cor}
\begin{rem}
	In fact, we can easily check that $\St_{\A} \subset \st{\A}$ is a $wk^{*}$-compact convex subset. And this corollary is equivalent to saying that $\St_{}$ can separate the points in $\A_{+}$.
\end{rem}
...
\end{proof}
\begin{rem}
	Therefore, any \Cs can be isometrically embedded in a operator algebra on some Hilbert space. Researching a abstract \Cs is actually researching the $\st{C}$-subalgebra of an operator algebra on some Hilbert space.
\end{rem}
...
\end{defn}
\begin{rem}
	Since $\pi$ is $*$-homomorphism and $\A$ is $*$-closed, the definition of irreducibility of $\pi$ is valid. In fact, if $\pi$ is algebraically irreducible, then $\pi$ is clearly topologically irreducible. But the converse is also true. Then we have,
	\begin{center}
...
We have known that each $\st{C}$-algebra can be isometrically embedded a $\oper$ for some Hilbert space $\Hs$. Therefore, we just research operator algebras other than $\st{C}$-algebras. Firstly, we need some general theories of operators on a Hilbert space.
...
By above proposition, we see the spectrum of operators can be described by using another method. Firstly, we can generalize the consequence in \textbf{Proposition} \ref{prop9}.
...
\begin{defn}
	...
	\begin{enumerate}[label=\arabic*)]
		...
		\item The weak operator topology on $\oper$, denoted by $WOT$, is generated by a family of seminorms $\{p_{gh}\}_{g,h \in \Hs}$, where $p_{gh}(T) = \abs{\langle Tg,h \rangle}$.
	\end{enumerate}
\end{defn}
\begin{rem}
